                        ...
                        <div class="row">
                            ...
                            @if (getMediaFileExit($appdata, 'app_image'))
                                <div class="col-md-2 mb-2">
                                    ...
                                    @endphp
                                    <img id="app_image_preview" src="{{ getSingleMedia($appdata, 'app_image') }}"
                                        alt="#" class="attachment-image mt-1"
                                        style="background-color:{{ $extention == 'svg' ? $appdata->color : '' }}">
                                    <a class="text-danger remove-file"
                                        href="{{ route('remove.file', ['id' => $appdata->id, 'type' => 'app_image']) }}"
                            ...
                            <div class="form-group col-md-4">
                                <label class="form-control-label"
                                    for="app_image_full">{{ __('messages.app_image_full') }} </label>
                                <div class="custom-file">
                                    <input type="file" name="app_image_full" class="custom-file-input"
                                        accept="image/*">
                                    <label
                                        class="custom-file-label upload-label">{{ __('messages.choose_file', ['file' => __('messages.app_image_full')]) }}</label>
                                </div>
                                <span class="selected_file"></span>
                            </div>
                            ...
                            @if (getMediaFileExit($appdata, 'app_image_full'))
                                <div class="col-md-2 mb-2">
                                    ...
                                    @endphp
                                    <img id="app_image_full_preview"
                                        src="{{ getSingleMedia($appdata, 'app_image_full') }}" alt="#"
                                        class="attachment-image mt-1"
                                        style="background-color:{{ $extention == 'svg' ? $appdata->color : '' }}">
                                    <a class="text-danger remove-file"
                                        href="{{ route('remove.file', ['id' => $appdata->id, 'type' => 'app_image_full']) }}"
                                        data--submit="confirm_form" data--confirmation='true' data--ajax="true"
                                        title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                        data-title='{{ __('messages.remove_file_title', ['name' => __('messages.image')]) }}'
                                        data-message='{{ __('messages.remove_file_msg') }}'>
